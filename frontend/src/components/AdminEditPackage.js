...
const AdminEditPackage = ({
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch(SummaryApi.update_package.url, {
      method: SummaryApi.update_package.method,
      credentials: 'include',
      headers: {
    ...
    <div className='fixed top-0 bottom-0 left-0 right-0 pt-5 pl-80 w-full h-full z-10 flex justify-left items-center bg-blue-200 bg-opacity-50'>
      <div className='bg-white text-black p-4 rounded w-full max-w-2xl h-full max-h-[85%] overflow-hidden'>
        <div className='flex justify-between items-center pb-3'>
          <h2 className='font-bold text-lg'>Update offer</h2>
          <div
        ...
        <form className='grid p-1 gap-1 overflow-y-scroll h-full pt-3 pb-5' onSubmit={handleSubmit}>
          ...
          <label htmlFor='packtName' className='mt-3'>Package Name :</label>
          <input
            type='text'
            id='packtName'
          ...
          <label htmlFor='packImage' className='mt-3'>Image :</label>
          ...
          <input
            ...
            required
          />
          <button className='px-3 py-2 bg-green-500 text-white hover:bg-rose-700'>Save</button>
        </form>
      </div>
