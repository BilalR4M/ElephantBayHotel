...
// console.log(dt)
    if (task.length > 0) {
        ...
        const startDate = task[0].startdate.split('T')[0];
        const endDate = task[0].enddate.split('T')[0];
...
<input type="date" id="dateInput" name="startdate" pattern="\d{4}-\d{2}-\d{2}" value={addTaskData.startdate} placeholder="YYYY-MM-DD" required onChange={handleInput}/>
</div>
...
<div> 
<p>Enter End Date</p>
<input type="date" id="enddate" name="enddate" pattern="\d{4}-\d{2}-\d{2}" value={addTaskData.enddate} placeholder="YYYY-MM-DD" required onChange={handleInput}/>
</div>
